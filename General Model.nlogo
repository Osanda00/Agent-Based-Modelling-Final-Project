globals [stable-tick-counter] ;; A counter for stable ticks
turtles-own
[
  Idea_state         ;; Idea1/2 or Neutral (Idea State of the turtle)
  ;; 0 - Neutral idea
  ;; 1 - Idea 1
  ;; 2 - Idea 2 ...
  next_Idea_state ;; Storing the Idea_state of the next tick
]
...
to setup
  clear-all
  setup-individuals ;; creating turtles and randomly placing them
  setup-spatially-clustered-network ;; creating links among turtles
  ask n-of initial-idea1-holders turtles [become-idea1-holder] ;; assigning initial idea 1 holders
  ask n-of initial-idea2-holders turtles with [Idea_State != 1] [become-idea2-holder] ;;assigning initial idea 2 holders
  set stable-tick-counter 0  ;; Initialize the counter for stable ticks
  reset-ticks
end
...
to setup-individuals
  ...
  create-turtles number-of-individuals ;; Creating individuals
  [
    ; for visual reasons, we don't put any nodes *too* close to the edges
    setxy (random-xcor * 0.95) (random-ycor * 0.95)
    become-neutral  ;; Initializing each individual in neutral state
  ]
end

to setup-spatially-clustered-network
  let num-links (average-connections-per-individual * (number-of-individuals)) / 2
  while [count links < num-links ]
  [
    ask one-of turtles
    [
      let choice (min-one-of (other turtles with [not link-neighbor? myself])
                   [distance myself]) ;;finding the turtle, without a link with considered turtle and has the minimum distance from it
  ...
  repeat 10
  [
    layout-spring turtles links 0.3 (world-width / (sqrt (number-of-individuals))) 1 ;; arrange turtles in a way that, nodes don't overlap and minimize link length
  ]
...
to go
  ;; Check if any turtle's Idea-state has changed
  ifelse all? turtles [Idea_state = next_Idea_state] [
    set stable-tick-counter stable-tick-counter + 1] [
    set stable-tick-counter 0 ] ;; Reset counter if there is a change

  ;; If stable for 20 consecutive ticks, stop the simulation
  if stable-tick-counter >= 5 [
    ;;user-message "Simulation has been stable for 20 ticks."
    stop
  ]

  spread-idea ;;finding what is the next_idea_state
  ask turtles
  [
   if Idea_state = 0 [
    if next_Idea_state = 1 [become-idea1-holder]
    if next_Idea_state = 2 [become-idea2-holder]]

   if Idea_state = 1 [
    if next_Idea_state = 1 [become-idea1-holder]
    if next_Idea_state = 2 and random-float 1 < 0.1 [become-idea2-holder]]

   if Idea_state = 2 [
    if next_Idea_state = 1 and random-float 1 < 0.1 [become-idea1-holder]
    if next_Idea_state = 2 [become-idea2-holder]]
  ]
...
to become-neutral  ;; turtle procedure for neutral idea
  set Idea_state 0
  set color blue
end

to become-idea1-holder  ;; turtle procedure for Idea 1
  set Idea_state 1
  set color red
end

to become-idea2-holder  ;; turtle procedure for Idea 2
  set Idea_state 2
  set color yellow
end
...
to spread-idea
  ask turtles
  [ let proportion-idea1 0
    let proportion-idea2 0

    if count link-neighbors != 0 [
    set proportion-idea1 ((count link-neighbors with [ Idea_state = 1 ])/(count link-neighbors))
    set proportion-idea2 ((count link-neighbors with [ Idea_state = 2 ])/(count link-neighbors))]



    let pass-ideas []  ;; Defining the empty list of possible accepting ideas

    if proportion-idea1 > threshold-idea1 [
      set pass-ideas lput 1 pass-ideas]
    if proportion-idea2 > threshold-idea2 [
      set pass-ideas lput 2 pass-ideas]

    if empty? pass-ideas [ set next_Idea_state Idea_state ]
    if length pass-ideas = 1 [ set next_Idea_state first pass-ideas ]
    if length pass-ideas > 1
    [
      if (proportion-idea1 - threshold-idea1) > (proportion-idea2 - threshold-idea2) [ set next_Idea_state 1 ]
      if (proportion-idea1 - threshold-idea1) < (proportion-idea2 - threshold-idea2) [ set next_Idea_state 2 ]
      if (proportion-idea1 - threshold-idea1) = (proportion-idea2 - threshold-idea2) [ set next_Idea_state one-of [1 2] ]
    ]
  ]
end
@#$#@#$#@
...
"" ""
PENS
"Neutral" 1.0 0 -13345367 true "" "plot (count turtles with [Idea_State = 0]) / (count turtles) * 100"
"Idea 1 Holders" 1.0 0 -2674135 true "" "plot (count turtles with [Idea_state = 1]) / (count turtles) * 100"
"Idea 2 Holders" 1.0 0 -1184463 true "" "plot (count turtles with [Idea_state = 2]) / (count turtles) * 100"
...
227
83
average-connections-per-individual
average-connections-per-individual
1
40
8.0
...
483
Neutral Count
count turtles with [Idea_State = 0]
17
1
11
...
482
Idea 1 Holders
count turtles with [Idea_state = 1]
17
1
11
...
483
Idea 2 Holders
count turtles with [ Idea_state = 2 ]
17
1
11
...
@#$#@#$#@
<experiments>
  <experiment name="Experiment test 1" repetitions="100" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>count turtles with [ Idea_state = 0 ] / count turtles</metric>
    <metric>count turtles with [ Idea_state = 1 ] / count turtles</metric>
    <metric>count turtles with [ Idea_state = 2 ] / count turtles</metric>
    <enumeratedValueSet variable="initial-idea1-holders">
      <value value="5"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="initial-idea2-holders">
      <value value="5"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="number-of-individuals">
      <value value="300"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="average-connections-per-individual">
      <value value="8"/>
    </enumeratedValueSet>
    <steppedValueSet variable="threshold-idea1" first="0" step="0.25" last="1"/>
    <steppedValueSet variable="threshold-idea2" first="0" step="0.25" last="1"/>
  </experiment>
  <experiment name="Experiment 1" repetitions="100" runMetricsEveryStep="false">
    ...
    <steppedValueSet variable="threshold-idea1" first="0" step="0.05" last="1"/>
    <steppedValueSet variable="threshold-idea2" first="0" step="0.05" last="1"/>
  </experiment>
  ...
  <experiment name="Experiment 5.1" repetitions="100" runMetricsEveryStep="false">
    ...
    <enumeratedValueSet variable="threshold-idea2">
      <value value="0.05"/>
      <value value="0.1"/>
      <value value="0.2"/>
      <value value="0.3"/>
  ...
  <experiment name="Experiment 3" repetitions="100" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>count turtles with [ Idea_state = 0 ] / count turtles</metric>
    <metric>count turtles with [ Idea_state = 1 ] / count turtles</metric>
  ...
  <experiment name="Experiment 6" repetitions="100" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <metric>count turtles with [ Idea_state = 0 ] / count turtles</metric>
    <metric>count turtles with [ Idea_state = 1 ] / count turtles</metric>
    ...
    <enumeratedValueSet variable="threshold-idea1">
      <value value="0.25"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="threshold-idea2">
      <value value="0.25"/>
  ...
  <experiment name="Experiment 7" repetitions="100" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <metric>count turtles with [ Idea_state = 0 ] / count turtles</metric>
    <metric>count turtles with [ Idea_state = 1 ] / count turtles</metric>
  ...
  <experiment name="Experiment 8" repetitions="100" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <metric>count turtles with [ Idea_state = 0 ] / count turtles</metric>
    <metric>count turtles with [ Idea_state = 1 ] / count turtles</metric>
  ...
  <experiment name="Experiment 9.1" repetitions="100" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <metric>count turtles with [ Idea_state = 0 ] / count turtles</metric>
    <metric>count turtles with [ Idea_state = 1 ] / count turtles</metric>
  ...
  <experiment name="Experiment 9.2" repetitions="100" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <metric>count turtles with [ Idea_state = 0 ] / count turtles</metric>
    <metric>count turtles with [ Idea_state = 1 ] / count turtles</metric>
  ...
  <experiment name="Experiment 9.3" repetitions="100" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <metric>count turtles with [ Idea_state = 0 ] / count turtles</metric>
    <metric>count turtles with [ Idea_state = 1 ] / count turtles</metric>
  ...
  <experiment name="Experiment 9.4" repetitions="100" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <metric>count turtles with [ Idea_state = 0 ] / count turtles</metric>
    <metric>count turtles with [ Idea_state = 1 ] / count turtles</metric>
  ...
  <experiment name="Experiment 8.1" repetitions="100" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <metric>count turtles with [ Idea_state = 0 ] / count turtles</metric>
    <metric>count turtles with [ Idea_state = 1 ] / count turtles</metric>
  ...
  <experiment name="Experiment 6.1" repetitions="1000" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
  ...
  <experiment name="Experiment 6.2" repetitions="1000" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <exitCondition>ticks = 60</exitCondition>
    <metric>count turtles with [ Idea_state = 0 ] / count turtles</metric>
  ...
  <experiment name="Experiment 6.3" repetitions="1000" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <exitCondition>ticks = 60</exitCondition>
    <metric>count turtles with [ Idea_state = 0 ] / count turtles</metric>
